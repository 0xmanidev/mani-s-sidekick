...
app.command("/manis-sidekick-ping", async ({ ack, respond }) => {
...
/manis-sidekick-todo add <task> - Add a todo
/manis-sidekick-todo list - Show your todos
/manis-sidekick-todo done <id> - Mark a todo as completed
/manis-sidekick-todo remove <id> - Delete a todo`,
  });
});
...
app.command("/manis-sidekick-todo", async ({ command, ack, respond }) => {
  ...
  switch (action) {
    case "add": {
      ...
      if (!task) {
        return respond(
          "❌ Usage: `/manis-sidekick-todo add <task>`"
        );
      }
...
#${todo.id}. ${todo.task}`,
    ...
    case "list": {
      ...
      if (userTodos.length === 0) {
        return respond("📭 You don't have any todos.");
      }

      const list = userTodos
        .map(
          (todo) =>
